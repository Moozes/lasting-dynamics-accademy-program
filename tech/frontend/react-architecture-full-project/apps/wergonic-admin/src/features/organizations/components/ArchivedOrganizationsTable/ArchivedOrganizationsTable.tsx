...
import { useColumns, useReadArchivedOrganizationsQuery } from "./ArchivedOrganizationsTable.hooks";

type Props = HTMLDivProps;

export const ArchivedOrganizationsTable = (props: Props) => {
    const t = useTranslationV2();
    const columns = useColumns();
    const [auth] = useAtom(authAtom);
    const { data, isLoading, isFetching, error, isError } = useReadArchivedOrganizationsQuery();
    ...
    if (isError) throw new Error("React query error thrown manually from ArchivedOrganizationsTable", { cause: error });
